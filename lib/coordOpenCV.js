...
class coordOpenCV {
  ...
  contours() {
    ...
    for (var i = 0; i < this.arrayOfContour.length; i++) {
      for (var j = 0; j < this.arrayOfContour.length; j++) {
        if (this.arrayOfContour[i].x > this.arrayOfContour[j].x - 5 &&
          this.arrayOfContour[i].x < this.arrayOfContour[j].x + 5 &&
          this.arrayOfContour[i].y > this.arrayOfContour[j].y - 5 &&
            this.arrayOfContour[i].y < this.arrayOfContour[j].y + 5 &&
            i!==j) {
          this.arrayOfContour.splice(i, 1)
        }
      }
    }
